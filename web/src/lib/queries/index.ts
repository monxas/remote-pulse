...
//
// The patch helpers are exported for vitest unit coverage; they take the
// raw cached payload (`old`) and the mutation input, and return a new
// payload that already has the change applied. That keeps the React-style
// onMutate body small and lets us assert on the patch in isolation.
...
export function patchUsersAfterCreate(
  ...
  // Placeholder id so the key tracking in {#each} stays happy until the
